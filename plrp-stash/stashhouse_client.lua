...
RegisterNetEvent('plrp-stash:setInitialState')
AddEventHandler('plrp-stash:setInitialState', function(stashesp)
   stashes = stashesp
end)

CreateThread(function()
    TriggerServerEvent("plrp-stash:fetchInitialState")
end)
...
RegisterNetEvent('plrp-stash:getnearstash')
AddEventHandler('plrp-stash:getnearstash', function()
    local closestDoorDistance, closestDoorId = 9999.9, -1
    local currentPos = GetEntityCoords(PlayerPedId())
    for id, handle in pairs(stashes) do
        local currentDoorDistance = #(stashes[id].StashEntry - currentPos)
...
RegisterNUICallback('complete', function(data, cb)
    CloseGui();
    cb('ok');
    TriggerEvent('plrp-stash:getnearstash')
end)
        ...
        if isinstash == true then
            ...
            if distance < 5 then
                ...
                if IsControlJustReleased(0, 38) then
                    ...
                    --TriggerEvent("apartments:client:OpenStash", 'stash' .. instash.ID)
                    TriggerEvent("inventory:client:SetCurrentStash", instash.ID)
                    TriggerServerEvent("inventory:server:OpenInventory", "stash", instash.ID, {
                        maxweight = 4000000,
                        slots = 500,
                    })
                else
